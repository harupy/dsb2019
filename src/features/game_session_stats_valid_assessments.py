...
from utils.common import remove_ext, prefix_list
...
def process_user_sample(user_sample, encoders, assess_titles, is_test_set=False):
    ...
    counter = 0  # this will increment on the assessment session.
    durations = []
    ...
    for key, session in user_sample.groupby('game_session', sort=False):
        ...
        contains_attempt = len(filter_assessment_attempt(session)) > 0
        if contains_attempt:
