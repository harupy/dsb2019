...
def make_profiles(data_dir):
    # read template source.
    with open(TEMPLATE_PATH, 'r') as f:
        template_src = f.read()

    files = search_by_extension(data_dir, ['.csv', '.ftr'])
    num_files = len(files)
    ...
    for file_idx, fpath in enumerate(files):
        ...
        # note that the output notebook will be placed in the same directory as the input data.
